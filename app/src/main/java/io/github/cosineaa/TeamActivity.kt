...
import io.github.cosineaa.TeamActivity.Companion.teamInstance
...
class TeamActivity : ComponentActivity() {
    ...
    companion object {
        lateinit var teamInstance: TeamActivity
    }
...
fun TeamStat() {
    Column(
        ...
        Row(horizontalArrangement = Arrangement.SpaceBetween) {
            ...
            val ppgRank = allTeamList.getPointsPerGameRank(teamInfo.teamKoreanShortName)
            TeamStatInfo(width = 0.5f, statName = "득점", rank = ppgRank, stat = ppgStat)

            val oppgStat = teamInfo.oppg.getStat()
            val oppgRank = allTeamList.getOpponentPointsPerGameRank(teamInfo.teamKoreanShortName)
            TeamStatInfo(width = 1f, statName = "실점", rank = oppgRank, stat = oppgStat)
        }
        Row(horizontalArrangement = Arrangement.SpaceBetween) {
            val reboundStat = teamInfo.rpg.getStat()
            val reboundRank = allTeamList.getReboundsPerGameRank(teamInfo.teamKoreanShortName)
            TeamStatInfo(width = 0.5f, statName = "리바운드", rank = reboundRank, stat = reboundStat)

            val assistStat = teamInfo.apg.getStat()
            val assistRank = allTeamList.getAssistsPerGameRank(teamInfo.teamKoreanShortName)
